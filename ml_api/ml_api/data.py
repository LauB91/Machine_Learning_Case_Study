...
def get_model_from_gcp():
    ...
    """
  #  with open(r"/Users/laurabonnet/Documents/GITHUBK/main-cyclist-337816-8df14917206d.json") as source:
  #     info = json.load(source)
    project_id = 'main-cyclist-337816'
  #  storage_credentials = service_account.Credentials.from_service_account_info(info)
  #  client = storage.Client(project=project_id, credentials=storage_credentials).bucket(BUCKET_NAME)
    client = storage.Client().bucket(BUCKET_NAME)
    local_model_name = 'model.joblib'
    model_storage_location = f"models/{MODEL_NAME}/{MODEL_VERSION}/{local_model_name}"
    blob = client.blob(model_storage_location)
